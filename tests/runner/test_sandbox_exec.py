...
def _run(
    ...
    *,
    ...
    if stdin is not None:
        (tmp_path / "in.txt").write_text(stdin)
    spec = SandboxSpec(
        workdir_host=str(tmp_path),
        command=command,
...
def test_clean_run_captures_stdout_and_usage(sandbox_image, tmp_path):
    result = _run(
        tmp_path,
...
def test_nonzero_exit_is_observed(sandbox_image, tmp_path):
    result = _run(tmp_path, ["python3", "-c", "import sys; sys.exit(3)"])
    assert result.exit_code == 3
    assert result.signal is None
    assert not result.exited_cleanly


def test_death_by_signal_is_observed(sandbox_image, tmp_path):
    result = _run(
        tmp_path,
...
def test_wall_clock_timeout_is_killed(sandbox_image, tmp_path):
    result = _run(tmp_path, ["python3", "-c", "while True: pass"], time_limit_ms=400)
...
def test_network_is_disabled(sandbox_image, tmp_path):
    ...
    result = _run(tmp_path, ["python3", "-c", code])
    assert result.exited_cleanly
    assert result.stdout.strip() == b"NO_NETWORK"


def test_memory_limit_triggers_oom(sandbox_image, tmp_path):
...
def test_output_is_capped(sandbox_image, tmp_path):
    result = _run(
        tmp_path,
...
def test_root_filesystem_is_read_only(sandbox_image, tmp_path):
